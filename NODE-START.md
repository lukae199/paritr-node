...
Diese Anleitung verwendet den Rust-Node `4.0.1-rc.3` und das neue Netzwerk
...
```text
paritr-node-4.0.1-rc.3-source.tar.gz
paritr-node-4.0.1-rc.3-source.tar.gz.sha256
```
...
```bash
cd ~
sha256sum -c paritr-node-4.0.1-rc.3-source.tar.gz.sha256
mkdir -p ~/paritr-mainnet-source
tar -xzf ~/paritr-node-4.0.1-rc.3-source.tar.gz -C ~/paritr-mainnet-source --strip-components=1
