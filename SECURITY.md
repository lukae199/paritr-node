...
Admin-API und Verwaltungsseite teilen standardmäßig den LAN-Listener `0.0.0.0:5051`. Der Portal-Agent erreicht dessen API intern über `127.0.0.1:5051`. Jede Zustandsänderung und jede nicht öffentliche Information benötigt das zufällige Admin-Secret. Für direkt registrierte Nodes stehen dieselben Secret-geschützten `/admin/*`-Routen auch am öffentlichen Listener bereit; dieser darf extern nur über einen HTTPS-Reverse-Proxy mit Rate-Limits veröffentlicht werden. Der Portal-Agent baut ausschließlich ausgehende HTTPS-Verbindungen auf und führt nur fest erlaubte RPC-Operationen aus. `admin_secret`, `node_private_key` und Portal-Token dürfen nie veröffentlicht werden.
...
- Management-Port 5051 ausschließlich im vertrauenswürdigen LAN freigeben und das Admin-Secret nicht in fremden Browsern speichern.
